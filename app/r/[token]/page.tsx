...
export async function generateMetadata({ params }: { params: Promise<{ token: string }> }): Promise<Metadata> {
  ...
  if (payload.kind === 'practice') {
    ...
    title = `${who}${pct}% у практиці CCA-F ${flair.emoji} — а ти?`;
    description = `Практичний челендж тренажера підготовки до Claude Certified Architect Foundations. Прийми виклик.`;
  } else {
    const tier = tierForScore(payload.score);
    title = `${who}${payload.score}/1000 на тренажері CCA-F ${tier.emoji}`;
  ...
  return {
    ...
    openGraph: { title, url: `https://cca.thevibe.works/r/${token}`, images: [{ url: ogImg, width: 1200, height: 630 }] },
...
export default async function SharePage({ params }: { params: Promise<{ token: string }> }) {
    ...
    return (
      <div className="page-wrap" style={{ maxWidth: 640 }}>
        <div className="card" style={{ padding: '40px 36px', textAlign: 'center' }}>
          {payload.nick && <div className="muted" style={{ fontSize: 18, marginBottom: 8 }}>{payload.nick} кидає виклик</div>}
          <div style={{ fontSize: 80, lineHeight: 1, marginBottom: 8 }}>{flair.emoji}</div>
          <div style={{ fontFamily: 'var(--disp)', fontSize: 64, fontWeight: 700, color: 'var(--accent)', lineHeight: 1 }}>{pct}%</div>
          <div className="muted" style={{ fontFamily: 'var(--mono)', fontSize: 14, marginTop: 6 }}>
            точність · {payload.correct}/{payload.total} правильних · {flair.label}
          </div>
          ...
          <div style={{ marginTop: 28 }}>
            <Link href="/trenazher" className="btn accent" style={{ fontSize: 18, padding: '14px 26px' }}>🔥 Прийняти виклик →</Link>
          </div>
          <p className="footnote" style={{ marginTop: 24 }}>Практика на тренажері CCA-F · не офіційна сертифікація Anthropic</p>
        </div>
        <div style={{ marginTop: 20, textAlign: 'center' }}>
          <Link href="/" style={{ color: 'var(--accent)', fontFamily: 'var(--mono)', fontSize: 14 }}>← Що таке CCA Тренажер</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="page-wrap" style={{ maxWidth: 640 }}>
      <div className="card" style={{ padding: '40px 36px', textAlign: 'center' }}>
        {payload.nick && <div className="muted" style={{ fontSize: 18, marginBottom: 8 }}>{payload.nick}</div>}
        ...
        <div style={{ marginTop: 28 }}>
          <Link href="/trenazher" className="btn accent" style={{ fontSize: 18, padding: '14px 26px' }}>🎯 Спробуй сам →</Link>
        </div>
